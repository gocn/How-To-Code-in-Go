...
在`main()`函数内，你宣告了`Version`变量，然后打印[string](https://gocn.github.io/How-To-Code-in-Go/docs/08-An_Introduction_to_Working_with_Strings_in_Go)类型的`Version`：紧跟着 tab 的字符，`\t`，然后是声明的变量。
